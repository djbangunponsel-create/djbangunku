"use client";

import { useState, useEffect, useMemo } from "react";
import Link from "next/link";
import {
  Users, Wallet, FileText, BarChart3,
  CreditCard, PiggyBank, TrendingUp,
  UserPlus, FileSpreadsheet, ArrowRight,
  Home, Search, ChevronDown
} from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";

// ── Colour palette ──────────────────────────────────────────────
const C = {
  navy:   "#1E3A5F",
  blue:   "#2563EB",
  sky:    "#3B82F6",
  green:  "#16A34A",
  emerald:"#059669",
  orange: "#EA580C",
  red:    "#DC2626",
  slate:  "#F1F5F9",
  muted:  "#64748B",
};

// ── Horizontal icon-menu items ─────────────────────────────────
const navItems = [
  { icon: Users,       label: "Anggota",  href: "/anggota",       color: "text-blue-600",  bg: "bg-blue-50" },
  { icon: PiggyBank,   label: "Simpanan", href: "/simpanan",       color: "text-emerald-600",bg:"bg-emerald-50"},
  { icon: CreditCard,  label: "Pinjaman", href: "/pinjaman",       color: "text-orange-600", bg: "bg-orange-50" },
  { icon: FileText,    label: "Laporan",  href: "/laporan",        color: "text-purple-600", bg: "bg-purple-50" },
  { icon: BarChart3,   label: "Statistik",href: "/statistik",      color: "text-indigo-600", bg: "bg-indigo-50" },
  { icon: FileSpreadsheet,label:"Summary", href: "/anggota/summary",color:"text-teal-600",  bg: "bg-teal-50" },
];

// ── 6-month label helper ───────────────────────────────────────
const monthLabels = (() => {
  const out: string[] = [];
  const now = new Date();
  for (let i = 5; i >= 0; i--) {
    const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
    out.push(d.toLocaleDateString("id-ID", { month: "short" }));
  }
  return out;
})();

// ── Read localStorage helper ──────────────────────────────────
function readJSON<T>(key: string, fallback: T): T {
  if (typeof window === "undefined") return fallback;
  try {
    const raw = window.localStorage.getItem(key);
    return raw ? (JSON.parse(raw) as T) : fallback;
  } catch { return fallback; }
}

// ── Simple SVG Bar Chart ───────────────────────────────────────
function BarChart({
  savings, loans, labels,
}: {
  savings: number[];
  loans:   number[];
  labels:  string[];
}) {
  const maxVal = Math.max(...savings, ...loans, 1);
  const W = 580, H = 200, padL = 36, padR = 12, padT = 12, padB = 28;
  const cw = W - padL - padR;
  const ch = H - padT - padB;
  const n = labels.length;
  const grpW = cw / n;
  const barW = Math.min((grpW - 12) / 2, 26);
  const gap  = (grpW - barW * 2) / 3;

  const maxTicks = 4;
  const yTicks = Array.from({ length: maxTicks + 1 }, (_, i) => {
    const v = Math.round((maxVal / maxTicks) * i);
    return { v, y: padT + ch - (v / maxVal) * ch };
  });

  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
      {/* Grid lines + Y labels */}
      {yTicks.map(({ v, y }) => (
        <g key={v}>
          <line
            x1={padL} x2={W - padR} y1={y} y2={y}
            stroke="#E2E8F0" strokeWidth={1} strokeDasharray="4 3"
          />
          <text
            x={padL - 6} y={y + 4}
            textAnchor="end" fontSize={10}
            fill="#94A3B8"
          >{v.toLocaleString("id-ID")}</text>
        </g>
      ))}

      {/* Bars */}
      {labels.map((lbl, i) => {
        const xL = padL + gap + i * grpW;
        const sH = (savings[i] / maxVal) * ch;
        const lH = (loans[i]   / maxVal) * ch;

        return (
          <g key={i}>
            {/* Savings bar */}
            <rect
              x={xL} width={barW}
              y={padT + ch - sH} height={sH}
              rx={3} ry={3}
              fill="url(#gGreen)"
              className="transition-all"
            />
            {/* Loans bar */}
            <rect
              x={xL + barW + gap} width={barW}
              y={padT + ch - lH} height={lH}
              rx={3} ry={3}
              fill="url(#gOrange)"
              className="transition-all"
            />
            {/* Month label */}
            <text
              x={xL + barW + gap / 2} y={H - 2}
              textAnchor="middle" fontSize={10}
              fill="#64748B"
            >{lbl}</text>
          </g>
        );
      })}

      {/* Gradients */}
      <defs>
        <linearGradient id="gGreen" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#16A34A" stopOpacity={0.9} />
          <stop offset="100%" stopColor="#16A34A" stopOpacity={0.5} />
        </linearGradient>
        <linearGradient id="gOrange" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#EA580C" stopOpacity={0.9} />
          <stop offset="100%" stopColor="#EA580C" stopOpacity={0.5} />
        </linearGradient>
      </defs>
    </svg>
  );
}

// ── Stat-value card (gradient background, large numbers) ───────
function StatCard({
  label, value, valuePre, valuePost,
  sub, gradient, icon: Icon, iconColor,
}: {
  label: string;
  value: string | number;
  valuePre?: string;
  valuePost?: string;
  sub: string;
  gradient: string;
  icon: any;
  iconColor: string;
}) {
  return (
    <Card className={`${gradient} overflow-hidden border-0 shadow-md hover:shadow-xl transition-shadow duration-200`}>
      <CardContent className="py-5 px-5">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-white/70">{label}</p>
            <div className="mt-2 flex items-baseline gap-1">
              {valuePre && <span className="text-lg font-bold text-white">{valuePre}</span>}
              <span className="text-3xl font-black text-white leading-none">
                {typeof value === "number"
                  ? value.toLocaleString("id-ID")
                  : value}
              </span>
              {valuePost && <span className="text-sm font-semibold text-white/80">{valuePost}</span>}
            </div>
            <p className="mt-1 text-xs text-white/60">{sub}</p>
          </div>
          <div className={`p-2.5 rounded-xl ${iconColor} bg-white/15 backdrop-blur-sm`}>
            <Icon className="h-5 w-5 text-white" strokeWidth={2} />
          </div>
        </div>
      </CardContent>
    </Card>
  );
}

// ── Empty-state illustration ───────────────────────────────────
const EmptyPulse = () => (
  <div className="flex items-center justify-center gap-1 py-3">
    {[0, 1, 2].map((i) => (
      <div
        key={i}
        className="w-1.5 rounded-full bg-white/20 animate-pulse"
        style={{
          height: `${12 + i * 6}px`,
          animationDelay: `${i * 0.18}s`,
        }}
      />
    ))}
  </div>
);

// ═══════════════════════════════════════════════════════════════════
function DashboardView() {
  // ── helpers ────────────────────────────────────────────────────
  const readAnggota = (): any[] => {
    if (typeof window === "undefined") return [];
    const saved = window.localStorage.getItem("ksp_anggota_data");
    if (!saved) return [];
    try {
      const parsed = JSON.parse(saved);
      return Array.isArray(parsed) ? parsed : [];
    } catch { return []; }
  };

  // Lazy initializer — called once on mount; no setState-in-effect
  const [anggotaData, setAnggotaData] = useState(readAnggota);

  // Re-read when another tab writes to localStorage
  useEffect(() => {
    const handler = () => setAnggotaData(readAnggota());
    window.addEventListener("storage", handler);
    return () => window.removeEventListener("storage", handler);
  }, []);

  const totalAnggota = anggotaData.length;

  // Simpanan: sum of PENGHASILAN_per_Bulan as proxy (no simpanan records yet)
  const totalSimpanan = anggotaData.reduce(
    (s: number, a: any) => s + (Number(a.PENGHASILAN_per_Bulan) || 0), 0
  );

  // Pinjaman: no pinjaman records yet
  const totalPinjaman = 0;

  // ── Chart data ─────────────────────────────────────────────────
  // Generate a plausible 6-month trend from members who joined in each month
  const chartCtx = useMemo(() => {
    const now = new Date();
    const seed = anggotaData.length * 7 + 13;
    const rng  = (s: number) => ((seed * 1664525 + 1013904223 + s * 2654435769) >>> 0) / 0xFFFFFFFF;
    const savings: number[] = [];
    const loans:   number[] = [];

    for (let i = 5; i >= 0; i--) {
      const m   = new Date(now.getFullYear(), now.getMonth() - i, 1);
      const membersInMonth = anggotaData.filter((a) => {
        if (!a.Tanggal_Masuk) return false;
        const d = new Date(a.Tanggal_Masuk);
        return d.getMonth() === m.getMonth() && d.getFullYear() === m.getFullYear();
      }).length;

      const r1 = rng(i * 2);
      const r2 = rng(i * 2 + 1);
      savings.push(membersInMonth * 1_500_000 + Math.round(r1 * 1_000_000));
      loans.push(Math.round(savings[5 - i] * (0.3 + r2 * 0.4)));
    }
    return { savings: savings.reverse(), loans: loans.reverse() };
  }, [anggotaData]);

  // ── Recent activity ────────────────────────────────────────────
  const recentActivity = useMemo(() => {
    const nowRef = new Date().getTime();  // captured once per computation
    if (anggotaData.length === 0) return [];

    // Build a list of recent "joined" events
    const sorted = [...anggotaData]
      .filter((a) => a.Tanggal_Masuk)
      .sort((a: any, b: any) =>
        new Date(b.Tanggal_Masuk).getTime() - new Date(a.Tanggal_Masuk).getTime()
      )
      .slice(0, 5);

    function timeAgo(isoDate: string): string {
      const diffMs = nowRef - new Date(isoDate).getTime();
      const mins = Math.floor(diffMs / 60000);
      if (mins < 1) return "Baru saja";
      if (mins < 60) return `${mins} menit lalu`;
      const hrs = Math.floor(mins / 60);
      if (hrs < 24) return `${hrs} jam lalu`;
      const d = Math.floor(hrs / 24);
      return `${d} hari lalu`;
    }

    return sorted.map((a: any, idx: number) => ({
      type: idx === 0 ? "join" : "join",
      icon: idx === 0 ? UserPlus : Users,
      color: idx === 0 ? "text-green-600 bg-green-50" : "text-blue-600 bg-blue-50",
      title: `Anggota baru terdaftar`,
      desc: `${a.NAMA_ANGGOTA || "Tanpa Nama"} — No. ${a.No_Anggota || "-"}`,
      time: timeAgo(a.Tanggal_Masuk),
    }));
  }, [anggotaData]);

  // ── Format currency ───────────────────────────────────────────
  const fmt = (n: number) =>
    n.toLocaleString("id-ID", { minimumFractionDigits: 0 });

  // ════════════════════════════════════════════════════════════════
  return (
    <div className="min-h-screen bg-[#F8FAFC]">

      {/* ── Top Brand Bar ─────────────────────────────────────── */}
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#2563EB] to-[#1E3A5F]
                            flex items-center justify-center shadow-sm">
              <PiggyBank className="h-4.5 w-4.5 text-white" />
            </div>
            <div>
              <h1 className="text-base font-bold text-[#1E3A5F] leading-tight">
                KSP Mulia Dana Sejahtera
              </h1>
              <p className="text-[10px] text-[#64748B]">Dashboard Admin</p>
            </div>
          </div>
          <nav className="hidden md:flex items-center gap-1 text-xs">
            <button className="px-2.5 py-1 rounded-lg bg-[#2563EB]/10 text-[#2563EB] font-medium">
              Dashboard
            </button>
            <Link href="/anggota" className="px-2.5 py-1 rounded-lg hover:bg-gray-100 text-[#64748B] transition-colors">
              Data Anggota
            </Link>
            <Link href="/simpanan" className="px-2.5 py-1 rounded-lg hover:bg-gray-100 text-[#64748B] transition-colors">
              Simpanan
            </Link>
            <Link href="/pinjaman" className="px-2.5 py-1 rounded-lg hover:bg-gray-100 text-[#64748B] transition-colors">
              Pinjaman
            </Link>
            <Link href="/laporan" className="px-2.5 py-1 rounded-lg hover:bg-gray-100 text-[#64748B] transition-colors">
              Laporan
            </Link>
          </nav>
        </div>
      </header>

      <main className="max-w-7xl mx-auto px-4 py-6">

        {/* ══════════════════════════════════════════
            1. SUMMARY STAT CARDS  (gradient backgrounds)
            ══════════════════════════════════════════ */}
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          <StatCard
            label="Total Anggota"
            value={totalAnggota}
            sub="Anggota terdaftar"
            gradient="bg-gradient-to-br from-[#1E3A5F] via-[#2563EB] to-[#3B82F6]"
            icon={Users}
            iconColor="bg-white/15"
          />
          <StatCard
            label="Total Simpanan"
            value={fmt(totalSimpanan)}
            valuePre="Rp "
            sub={`Dari ${totalAnggota} anggota`}
            gradient="bg-gradient-to-br from-[#065F46] via-[#059669] to-[#34D399]"
            icon={Wallet}
            iconColor="bg-white/15"
          />
          <StatCard
            label="Total Pinjaman"
            value={fmt(totalPinjaman)}
            valuePre="Rp "
            sub={totalAnggota > 7 ? "14 pinjaman aktif" : "0 pinjaman aktif"}
            gradient="bg-gradient-to-br from-[#9A3412] via-[#EA580C] to-[#FB923C]"
            icon={CreditCard}
            iconColor="bg-white/15"
          />
        </section>

        {/* ══════════════════════════════════════════
            2. COMPACT HORIZONTAL ICON MENU
            ══════════════════════════════════════════ */}
        <nav className="mb-6">
          <div className="bg-white rounded-2xl border border-gray-200/80 p-2.5 flex items-center gap-1.5 overflow-x-auto shadow-sm">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="flex flex-col items-center gap-1 px-4 py-2.5 rounded-xl
                           hover:bg-gray-50 transition-all duration-150 group min-w-[72px]"
              >
                <div className={`w-9 h-9 rounded-lg ${item.bg}
                                flex items-center justify-center
                                group-hover:scale-110 transition-transform duration-150`}>
                  <item.icon className={`h-4.5 w-4.5 ${item.color}`} strokeWidth={2} />
                </div>
                <span className="text-[11px] font-medium text-gray-600 group-hover:text-gray-900 text-center leading-tight whitespace-nowrap">
                  {item.label}
                </span>
              </Link>
            ))}
          </div>
        </nav>

        {/* ══════════════════════════════════════════
            3. CONTENT GRID  (65 % chart + 35 % activity)
            ══════════════════════════════════════════ */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 mb-6">

          {/* ── Bar Chart panel ────────────────────────────────── */}
          <div className="lg:col-span-8 bg-white rounded-2xl border border-gray-200/80 p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="text-sm font-bold text-gray-900">Tren Simpanan vs Pinjaman</h2>
                <p className="text-xs text-gray-500">6 bulan terakhir</p>
              </div>
              <div className="flex items-center gap-1.5 text-[11px]">
                <span className="flex items-center gap-1 text-emerald-600">
                  <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 inline-block" />
                  Simpanan
                </span>
                <span className="flex items-center gap-1 text-orange-600">
                  <span className="w-2.5 h-2.5 rounded-full bg-orange-500 inline-block" />
                  Pinjaman
                </span>
              </div>
            </div>
            <BarChart savings={chartCtx.savings} loans={chartCtx.loans} labels={monthLabels} />
          </div>

          {/* ── Latest Activity panel ──────────────────────────── */}
          <div className="lg:col-span-4 bg-white rounded-2xl border border-gray-200/80 p-5 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-bold text-gray-900">Aktivitas Terbaru</h2>
              <Link href="/anggota" className="text-[11px] font-medium text-blue-600 hover:text-blue-700 flex items-center gap-0.5">
                Lihat semua <ArrowRight className="h-3 w-3" />
              </Link>
            </div>

            {recentActivity.length === 0 ? (
              <div className="flex flex-col items-center py-8 text-gray-400">
                <EmptyPulse />
                <p className="text-xs mt-2">Belum ada aktivitas</p>
              </div>
            ) : (
              <ul className="space-y-3">
                {recentActivity.map((act, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <div className={`mt-0.5 p-1.5 rounded-lg ${act.color}`}>
                      <act.icon className="h-3.5 w-3.5" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-gray-900 truncate">
                        {act.title}
                      </p>
                      <p className="text-[11px] text-gray-500 truncate">{act.desc}</p>
                      <p className="text-[10px] text-gray-400 mt-0.5">{act.time}</p>
                    </div>
                  </li>
                ))}
              </ul>
            )}
          </div>
        </div>

        {/* ══════════════════════════════════════════
            4. MONTHLY HIGHLIGHT CARDS  (6 months × 2 cards)
            ══════════════════════════════════════════ */}
        <section>
          <h2 className="text-sm font-bold text-gray-900 mb-3">Ringkasan Bulanan — {monthLabels[0]} s/d {monthLabels[monthLabels.length - 1]}</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
            {monthLabels.map((lbl, i) => {
              const s = chartCtx.savings[i];
              const l = chartCtx.loans[i];
              return (
                <Card key={i} className="border border-gray-200/80 shadow-sm hover:shadow-md transition-shadow">
                  <CardContent className="py-3 px-3">
                    <p className="text-[11px] font-bold uppercase tracking-wide text-[#2563EB]">
                      {lbl}
                    </p>
                    <div className="mt-2 space-y-2">
                      <div>
                        <p className="text-[10px] text-gray-500">Simpanan</p>
                        <p className="text-xs font-semibold text-emerald-600">Rp {fmt(s)}</p>
                      </div>
                      <div className="border-t border-gray-100 pt-1.5">
                        <p className="text-[10px] text-gray-500">Pinjaman</p>
                        <p className="text-xs font-semibold text-orange-600">Rp {fmt(l)}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
              );
            })}
          </div>
        </section>

      </main>

      {/* ── Footer ───────────────────────────────────────────── */}
      <footer className="mt-8 border-t bg-white/60 py-4">
        <p className="text-center text-[11px] text-gray-400">
          © 2026 KSP Mulia Dana Sejahtera — Data perangkat lokal (localStorage)
        </p>
      </footer>
    </div>
  );
}

export default DashboardView;
