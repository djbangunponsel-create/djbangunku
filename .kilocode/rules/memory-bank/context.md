...
|  |
|  | 2026-05-22 | **Diagnosis & fix round 2 — Pengaturan/Neraca persist after sidebar fix**:
|  | - [x] **Pengaturan page code audit**: `src/app/pengaturan/page.tsx` is a pure server component (no `use client`). `PengaturanClientContent.tsx` (`use client`) cursorily reviewed — no `from 'next'` imports, only `react`, `lucide-react`, `@/components/ui/*`, `@/lib/storage/*`. All `@/lib/storage` calls guarded with `typeof window === 'undefined'` guard in SSR paths. Client component itself is structurally valid.
|  | - [x] **`src/app/pengaturan/page.tsx`**: added `export const metadata: Metadata` (matching pattern of all other working pages). `bun typecheck` + `bun run build` both clean. Production build confirmed all 24 routes render — `/pengaturan` shows as `○` (static, pre-rendered).
