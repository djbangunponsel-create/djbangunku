...
import { getPengaturan } from '@/lib/storage-db';
...
export default function NeracaPage() {
  const [settings, setSettings] = useState<KspSettings>(_readSettings());
  
  function _readSettings(): KspSettings {
    try {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        const remoteSettings = await getPengaturan();
        if (remoteSettings && Object.keys(remoteSettings).length > 0) {
          setSettings({
            ..._kspSettingsDefault,
            ...remoteSettings,
            penjamin: remoteSettings.penjamin ?? [],
          });
        }
      } catch { /* ignore */ }
    };
    ...
    const handler = () => {
      const saved = readStored<KspSettings>(KEYS.SETTINGS, _kspSettingsDefault);
      setSettings(saved);
    };
  ...
  const KSP_NAMA   = settings.namaKsp    || _kspSettingsDefault.namaKsp;
  const KSP_ALAMAT = settings.alamat    || _kspSettingsDefault.alamat;
  const KSP_LOGO   = settings.logo;
  const KSP_TELP   = settings.telepon  || '';
  ...
  // ── Ekuitas ─────────────────────────────────────────────────────
  const cModalAnggotaTetap     = c.Pokok;
  const cModalAnggotaDiperlukan = c.Wajib;
  const cTotalEkuitas           = cModalAnggotaTetap + cModalAnggotaDiperlukan;
          ...
          <CardHeader>
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div>
                ...
                <CardDescription>
                  {KSP_NAMA} — Perbandingan: {yearConfig.prevYear} | {yearConfig.currentYear}
                </CardDescription>
              </div>
